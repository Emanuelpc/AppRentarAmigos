...
function BuscadorAmigo() {
    ...
    const BuscarAmigoNombre = () => {
      ...
      Axios.get("http://localhost:3306/amigoBusqueda", {
              params: {
                Nombre:resultado[0],
    ...
    const getAmigo = () => {
    Axios.get("http://localhost:3306/amigosconDepartamento").then((response) => {
      setamigos(response.data);
    });
    }
    // Función para obtener la lista de Departamentos del backend
    const getDepartamentos = () => {
      Axios.get("http://localhost:3306/departamentos").then((response) => {
        setdepartamentos(response.data);
      });
    }
    // Función para obtener la lista de Ciudades del backend
    const getCiudades = () => {
      Axios.get("http://localhost:3306/ciudades").then((response) => {
    ...
    const handleBuscarClick = () => {
      ...
      if (guardarSeleccionComboboxDepartamento.length === 0 &&
          ...
          Axios.get("http://localhost:3306/amigosfiltrado", {
              params: {
